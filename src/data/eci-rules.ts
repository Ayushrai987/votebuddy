// ============================================================
// VoteBuddy - ECI Rules Data
// ============================================================

export interface ECIRuleCategory {
  id: string;
  title: string;
  titleHi: string;
  icon: string;
  summary: string;
  points: string[];
  plainEnglish: string;
  extras?: Record<string, string>;
}

export const eciRules: Record<string, ECIRuleCategory> = {
  mcc: {
    id: "mcc",
    title: "Model Code of Conduct",
    titleHi: "आदर्श आचार संहिता",
    icon: "",
    summary:
      "Rules that govern the behavior of political parties and candidates during elections",
    plainEnglish:
      "The Model Code of Conduct (MCC) is a set of guidelines issued by ECI for political parties and candidates during elections. It kicks in the moment elections are announced and stays until results are declared. Think of it as the 'fair play rulebook' - no hate speech, no bribing voters, no misusing government resources, and no making promises you can't keep with public money.",
    points: [
      "No aggravation of caste/communal differences for votes",
      "Criticism limited to policies and track record - no personal attacks",
      "No appeal to caste or communal feelings for votes",
      "No government machinery or resources for campaigning",
      "No new project announcements or inaugurations after MCC",
      "No official media (Doordarshan/AIR) for party propaganda",
      "No campaign office or rally within 200m of polling booth",
      "All political ads need MCMC pre-certification",
      "No exit polls during the entire polling period",
      "Ministers cannot combine official visit with election work",
      "No transfer of officers without ECI permission",
      "No use of religious places for election propaganda",
    ],
  },
  nomination: {
    id: "nomination",
    title: "Nomination & Scrutiny Rules",
    titleHi: "नामांकन और जांच नियम",
    icon: "",
    summary:
      "Rules governing who can contest elections and how to file nominations",
    plainEnglish:
      "To fight an election, you need to be an Indian citizen, at least 25 years old, a registered voter, and not disqualified under any law. You file nomination papers with the Returning Officer, pay a deposit (₹25,000 for Lok Sabha), and submit an affidavit declaring your criminal cases, assets, and education. If you get less than 1/6th of total valid votes, you lose your deposit.",
    points: [
      "Must be a registered voter in any Indian constituency",
      "Minimum age: 25 years (Lok Sabha & Vidhan Sabha), 30 years (Rajya Sabha)",
      "Deposit: ₹25,000 / ₹12,500 for SC-ST (Lok Sabha)",
      "Deposit: ₹10,000 / ₹5,000 for SC-ST (Vidhan Sabha)",
      "Must be proposed by a registered voter of that constituency",
      "Mandatory affidavit declaring: criminal cases, assets, liabilities, education",
      "Maximum 4 nomination sets per candidate",
      "Deposit forfeited if candidate gets < 1/6th of valid votes polled",
      "Cannot contest from more than 2 constituencies simultaneously",
      "Scrutiny by Returning Officer within 1 day of nomination deadline",
    ],
  },
  campaign: {
    id: "campaign",
    title: "Campaign Rules & Restrictions",
    titleHi: "प्रचार नियम और प्रतिबंध",
    icon: "",
    summary: "What's allowed and prohibited during election campaigning",
    plainEnglish:
      "You can hold rallies, go door-to-door, use social media - but there are boundaries. All campaigning must stop 48 hours before polling (the 'silence period'). Loudspeakers only between 6 AM and 10 PM. No liquor distribution 48 hours before and on polling day. Every rally needs police permission. Social media posts are also under MCC scrutiny.",
    points: [
      "Campaign must stop 48 hours before polling day",
      "Loudspeakers allowed only between 6:00 AM and 10:00 PM",
      "No procession within 24 hours before polling",
      "Prior police permission required for all rallies and processions",
      "No liquor distribution 48 hours before and on polling day",
      "No paid news - all ads need MCMC certification",
      "Social media campaigns are under MCC surveillance",
      "Campaign vehicles need district permits",
      "No defacement of public or private property without permission",
      "No appeal to religion, caste, or community for votes",
      "No cash or gift distribution to voters",
    ],
  },
  expenditure: {
    id: "expenditure",
    title: "Election Expenditure Limits",
    titleHi: "चुनाव खर्च सीमा",
    icon: "",
    summary: "Maximum spending limits for candidates and parties",
    plainEnglish:
      "Every candidate has a spending cap. For Lok Sabha, it's ₹95 lakh in larger states and ₹75 lakh in smaller states. For Vidhan Sabha, it's ₹40 lakh (large) and ₹28 lakh (small). You must maintain day-wise accounts and submit them within 30 days of results. Expenditure observers shadow candidates to verify. Party spending is separate but also monitored.",
    points: [
      "Lok Sabha: ₹95 Lakh (large states) / ₹75 Lakh (small states)",
      "Vidhan Sabha: ₹40 Lakh (large states) / ₹28 Lakh (small states)",
      "Day-wise expenditure accounts are mandatory",
      "Must submit accounts to DEO within 30 days of result",
      "Includes travel, accommodation, media, rallies, printing",
      "Party expenditure tracked separately",
      "Shadow observation by Expenditure Observers",
      "Exceeding limit can lead to disqualification",
    ],
    extras: {
      lokSabha: "₹95 Lakh (large states) / ₹75 Lakh (small states)",
      vidhanSabha: "₹40 Lakh (large states) / ₹28 Lakh (small states)",
    },
  },
  evmVvpat: {
    id: "evmVvpat",
    title: "EVM & VVPAT System",
    titleHi: "ईवीएम और वीवीपैट प्रणाली",
    icon: "",
    summary: "How Electronic Voting Machines and paper audit trail work",
    plainEnglish:
      "India uses Electronic Voting Machines (EVMs) for voting. Each EVM has two parts: a Ballot Unit (where you press the button) and a Control Unit (held by the Presiding Officer). After you vote, a VVPAT machine prints a small paper slip showing who you voted for - visible for 7 seconds through a window. EVMs are completely standalone with no internet or wireless connectivity. In every constituency, 5 random booths' VVPAT slips are counted and matched with EVM results.",
    points: [
      "Two units: Ballot Unit (voter) + Control Unit (officer)",
      "Maximum capacity: 2000 votes, up to 384 candidates",
      "VVPAT prints slip with candidate name, symbol, serial number",
      "7-second viewing window for voter to verify",
      "5 random VVPAT booths counted per constituency for verification",
      "Mock poll: minimum 50 votes before actual polling begins",
      "EVMs are fully standalone - no WiFi, no Bluetooth, no internet",
      "First Level Checking (FLC) done weeks before election",
      "VVPAT paper count is final if any discrepancy found",
      "EVMs manufactured only by BEL and ECIL (government companies)",
      "Two-stage randomization for EVM assignment to booths",
    ],
  },
  observers: {
    id: "observers",
    title: "Election Observers",
    titleHi: "चुनाव पर्यवेक्षक",
    icon: "",
    summary:
      "Types of observers deployed by ECI to ensure free and fair elections",
    plainEnglish:
      "ECI deploys senior officers from outside the state as observers. There are 4 types: General Observers (overall process), Expenditure Observers (track candidate spending), Police Observers (security), and Micro Observers (deployed at sensitive booths). They report directly to ECI, not to the state government.",
    points: [
      "General Observer - oversees the entire election process in assigned constituency",
      "Expenditure Observer - tracks candidate and party spending",
      "Police Observer - ensures security arrangements and law enforcement",
      "Micro Observer - deployed at specific sensitive polling booths",
      "All observers are IAS/IPS/IRS officers from outside the state",
      "Report directly to Election Commission of India",
      "Cannot be influenced by local administration",
      "One General Observer for every 2-3 constituencies",
    ],
  },
  pollingDay: {
    id: "pollingDay",
    title: "Polling Day Rules",
    titleHi: "मतदान दिवस नियम",
    icon: "",
    summary: "Rules for voters, agents, and officials on election day",
    plainEnglish:
      "On polling day, carry your voter ID, check your booth number in advance, and reach before closing time (usually 6 PM). You cannot carry phones inside the voting compartment, wear party symbols, or take selfies with the EVM. Within 100 meters of the booth, no campaigning is allowed. Polling agents of candidates can be present but cannot interfere.",
    points: [
      "Polling hours: typically 7:00 AM to 6:00 PM",
      "Must carry valid photo ID (EPIC or 12 alternatives)",
      "No mobile phones inside the voting compartment",
      "No wearing of party symbols or colors inside booth premises",
      "No photography or selfies with EVM",
      "No campaigning within 100 meters of polling booth",
      "No campaign material within 200 meters of booth",
      "Free transport not allowed by parties on polling day",
      "Violation of booth rules is a criminal offense",
      "Presiding Officer is the final authority at the booth",
    ],
  },
  paidNews: {
    id: "paidNews",
    title: "Media & Paid News Rules",
    titleHi: "मीडिया और पेड न्यूज़ नियम",
    icon: "",
    summary: "Rules governing media coverage and paid political advertisements",
    plainEnglish:
      "Paid news disguised as editorial content is illegal. All political ads in media must carry a 'PAID ADVERTISEMENT' label. Each district has a Media Certification and Monitoring Committee (MCMC) that reviews ads. Social media ads must have transparent funding info. Violations can lead to disqualification under RPA. No exit polls are allowed during the polling period.",
    points: [
      "Paid news treated as election expenditure of the candidate",
      "MCMC (Media Certification & Monitoring Committee) set up in each district",
      "All political ads must carry 'PAID ADVERTISEMENT' label",
      "No political advertisement disguised as editorial/news content",
      "Social media political ads must have transparent funding",
      "Exit polls prohibited from start of polling to 30 minutes after last phase ends",
      "Violations can lead to disqualification under RPA",
      "Print and electronic media under same rules",
    ],
  },
  nota: {
    id: "nota",
    title: "NOTA - None of the Above",
    titleHi: "नोटा - उपरोक्त में से कोई नहीं",
    icon: "",
    summary: "The option to reject all candidates",
    plainEnglish:
      "Since 2013, Indian voters have the option to press NOTA - 'None of the Above' - if they don't want to vote for any candidate. It's the last button on the EVM. While NOTA votes are counted and reported, even if NOTA gets the most votes, the candidate with the next highest votes still wins. It's essentially a form of protest voting.",
    points: [
      "Introduced by Supreme Court in September 2013",
      "Last button on the EVM ballot unit",
      "Allows voter to reject all candidates",
      "NOTA votes are counted and publicly reported",
      "Even if NOTA gets most votes, the highest-voted candidate wins",
      "Acts as a form of democratic protest",
      "Available in all elections - LS, VS, and local body",
      "NOTA symbol is a ballot paper with a cross mark",
    ],
  },
};

export const voterRegistration = {
  form6: {
    id: "form6",
    title: "Form 6 - New Voter Registration",
    titleHi: "फॉर्म 6 - नया मतदाता पंजीकरण",
    icon: "",
    summary: "Register as a new voter in your constituency",
    steps: [
      "Check eligibility: Indian citizen, 18+ by qualifying date",
      "Visit voters.eci.gov.in or download Voter Helpline App",
      "Fill Form 6 online with personal details",
      "Upload documents: age proof, address proof, passport photo",
      "Submit and note application reference number",
      "BLO will visit for verification within 2-4 weeks",
      "Check status online or via helpline 1950",
      "Receive EPIC (Voter ID) after approval",
    ],
    documents: [
      "Age proof: Birth certificate, 10th marksheet, passport",
      "Address proof: Aadhaar, ration card, utility bill, passport",
      "Passport-size photograph",
    ],
    qualifyingDates: ["January 1", "April 1", "July 1", "October 1"],
  },
  form7: {
    id: "form7",
    title: "Form 7 - Objection / Deletion",
    titleHi: "फॉर्म 7 - आपत्ति / विलोपन",
    icon: "",
    summary: "Request deletion or raise objection against an entry",
    details: [
      "Used to request deletion or raise objection to an electoral roll entry",
      "For shifted, deceased, or duplicate entries",
      "Any registered voter in the constituency can file",
      "ERO gives 7 days notice before deletion",
      "Helps keep electoral rolls clean and updated",
    ],
  },
  form8: {
    id: "form8",
    title: "Form 8 - Correction of Details",
    titleHi: "फॉर्म 8 - विवरण में सुधार",
    icon: "",
    summary: "Correct errors in your voter registration details",
    details: [
      "Form 8A: Correct name, age, photo, address, gender",
      "Form 8B: Transposition within same constituency (different part)",
      "Available online via NVSP portal",
      "BLO verification required for address changes",
      "No charges for any corrections",
    ],
  },
  epic: {
    id: "epic",
    title: "EPIC - Voter ID Card",
    titleHi: "EPIC - मतदाता पहचान पत्र",
    icon: "",
    summary: "Your Elector's Photo Identity Card",
    details: [
      "Electors Photo Identity Card - primary voter ID",
      "Issued free of cost by Election Commission",
      "Contains: name, photo, DOB, address, EPIC number",
      "e-EPIC downloadable from voters.eci.gov.in",
      "12 alternative IDs accepted at polling booth if no EPIC",
      "Format: 3 letters + 7 digits (e.g., ABC1234567)",
    ],
    alternativeIds: [
      "Aadhaar Card",
      "Passport",
      "Driving License",
      "PAN Card",
      "Service ID of Central/State Govt",
      "Student ID with photo",
      "Bank/Post Office passbook with photo",
      "Smart Card issued by RGI",
      "MNREGA Job Card",
      "Health Insurance Smart Card",
      "Pension document with photo",
      "MP/MLA/MLC identity card",
    ],
  },
  helpline: {
    id: "helpline",
    title: "Voter Helpline 1950",
    titleHi: "मतदाता हेल्पलाइन 1950",
    icon: "",
    summary: "Toll-free helpline for all voter services",
    details: [
      "Toll-free number: 1950",
      "Available in Hindi, English, and regional languages",
      "Services: status check, booth finder, complaint filing",
      "Voter Helpline App available on Android & iOS",
      "cVIGIL app for reporting MCC violations",
      "NVSP portal: voters.eci.gov.in",
      "Available 24/7 during election season",
    ],
  },
};

export const electionProcess = [
  {
    step: 1,
    title: "Announcement",
    titleHi: "चुनाव की घोषणा",
    icon: "",
    desc: "ECI announces election schedule with dates for nomination, scrutiny, withdrawal, and polling.",
    duration: "45-60 days before first poll",
  },
  {
    step: 2,
    title: "Model Code of Conduct",
    titleHi: "आदर्श आचार संहिता",
    icon: "",
    desc: "MCC comes into effect immediately upon announcement. All parties must follow strict guidelines.",
    duration: "Immediate",
  },
  {
    step: 3,
    title: "Filing Nominations",
    titleHi: "नामांकन दाखिल",
    icon: "",
    desc: "Candidates file papers with Returning Officer. Deposit: ₹25,000 General / ₹12,500 SC-ST (LS).",
    duration: "~7 day window",
  },
  {
    step: 4,
    title: "Scrutiny",
    titleHi: "नामांकन जांच",
    icon: "",
    desc: "Returning Officer verifies all nomination papers for eligibility and validity.",
    duration: "1 day after deadline",
  },
  {
    step: 5,
    title: "Withdrawal",
    titleHi: "नामांकन वापसी",
    icon: "",
    desc: "Candidates may withdraw nominations before the deadline.",
    duration: "2 days after scrutiny",
  },
  {
    step: 6,
    title: "Campaigning",
    titleHi: "चुनाव प्रचार",
    icon: "",
    desc: "Rallies, media campaigns, door-to-door canvassing. Must stop 48 hours before polling.",
    duration: "2-3 weeks",
  },
  {
    step: 7,
    title: "Silence Period",
    titleHi: "मौन अवधि",
    icon: "",
    desc: "No campaigning allowed. Voters reflect on their choice.",
    duration: "48 hours",
  },
  {
    step: 8,
    title: "Polling Day",
    titleHi: "मतदान दिवस",
    icon: "",
    desc: "Voters cast votes via EVMs at designated polling booths. Typically 7 AM to 6 PM.",
    duration: "1 day per phase",
  },
  {
    step: 9,
    title: "Counting",
    titleHi: "मतगणना",
    icon: "",
    desc: "Postal ballots counted first, then EVM votes round by round at designated counting centers.",
    duration: "1-2 days",
  },
  {
    step: 10,
    title: "Results & Formation",
    titleHi: "परिणाम और सरकार गठन",
    icon: "",
    desc: "Results declared constituency-wise. Majority party/coalition invited to form government.",
    duration: "Same day + weeks",
  },
];

export const boothRules = {
  voterLimit:
    "Maximum 1500 voters per polling booth. Auxiliary booth created if exceeded.",
  mandatoryFacilities: [
    "Ramp for disabled/elderly (wheelchair accessible)",
    "Drinking water facility",
    "Shade/shelter for voters in queue",
    "VVPAT with every EVM",
    "Separate queue for women, 65+, PwD",
    "Voter assistance booth",
    "Toilet facility",
    "Adequate lighting",
    "First aid kit",
    "Signage in local language",
    "Braille signage for visually impaired",
  ],
  officials: {
    blo: {
      title: "BLO (Booth Level Officer)",
      icon: "",
      duties: [
        "Door-to-door voter list verification",
        "Assists in new voter registration",
        "Distributes voter slips before election",
        "Updates Electoral Roll continuously",
        "Reports to Electoral Registration Officer (ERO)",
        "Handles approximately 1000-1500 voters",
      ],
    },
    sectorOfficer: {
      title: "Sector Officer",
      icon: "",
      duties: [
        "Supervises 10-15 polling booths",
        "Ensures election material delivery and collection",
        "Monitors polling process and reports issues",
        "Coordinates with all Presiding Officers in sector",
        "Ensures mock poll completion before actual polling",
        "Reports directly to Returning Officer",
      ],
    },
    presidingOfficer: {
      title: "Presiding Officer",
      icon: "",
      duties: [
        "In-charge of the polling booth",
        "Conducts mock poll at 5:30 AM on polling day",
        "Ensures free and fair polling throughout the day",
        "Handles Form 17A (voter register)",
        "Seals EVM after polling concludes",
        "Prepares final voter account (Form 17C)",
      ],
    },
  },
  locationCriteria: [
    "Government building preferred (school, panchayat bhavan)",
    "Within 2 km walking distance for voters",
    "Road accessible (especially for PwD)",
    "Not a religious place",
    "Not private property without permission",
    "Adequate space as per ECI specifications",
    "Proper security arrangements possible",
    "Not near any political party office",
  ],
};
