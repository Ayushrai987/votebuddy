        ...
        {phases.map((p, i) => (
          <div key={i} className="relative flex items-start gap-4 mb-6 pl-12">
            ...
            >
              {i < activePhase && (
                <svg
                  className="w-3 h-3 text-saffron-500"
                  fill="currentColor"
                  viewBox="0 0 20 20"
                >
                  <path
            ...
            <div
              className={`flex-1 ${i === activePhase ? "opacity-100" : "opacity-50"}`}
            >
    ...
    <div className="page-enter mx-auto max-w-7xl px-4 sm:px-6 py-8 sm:py-12">
            ...
            <div className="flex items-center justify-between flex-wrap gap-4 mb-4">
              <div>
                ...
                <p className="text-sm text-[var(--text-secondary)] mt-1">
                  {lokSabha2024.totalSeats} {t("elections.seats")} -{" "}
                  {lokSabha2024.phases.length} {t("states.phases")}
                </p>
              </div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-8">
              <div className="stat-card">
                <span className="text-xs text-[var(--text-muted)] uppercase tracking-wider font-semibold">
                  {t("elections.notification")}
                </span>
                <p className="text-sm font-bold text-[var(--text-primary)] mt-1">
                  {lokSabha2024.notificationDate}
                </p>
              </div>
              <div className="stat-card">
                <span className="text-xs text-[var(--text-muted)] uppercase tracking-wider font-semibold">
                  {t("elections.firstPoll")}
                </span>
                <p className="text-sm font-bold text-[var(--text-primary)] mt-1">
                  {lokSabha2024.phases[0].date}
                </p>
              </div>
              <div className="stat-card">
                <span className="text-xs text-[var(--text-muted)] uppercase tracking-wider font-semibold">
                  {t("elections.lastPoll")}
                </span>
                <p className="text-sm font-bold text-[var(--text-primary)] mt-1">
                  {lokSabha2024.phases[6].date}
                </p>
              </div>
              <div className="stat-card">
                <span className="text-xs text-[var(--text-muted)] uppercase tracking-wider font-semibold">
        ...
        <div className="animate-fade-up">
          <div className="glass-card p-6 sm:p-8 mb-8">
            <h2 className="text-xl font-bold text-[var(--text-primary)] mb-2">
              {t("elections.vidhanSabha")}
            </h2>
            <p className="text-[var(--text-secondary)] max-w-md mb-6">
              {language === "en"
                ? "Tracking state elections for the 2026 cycle."
                : "2026 चक्र के लिए राज्य चुनावों पर नज़र रखना।"}
            </p>
            ...
            <h3 className="section-heading mb-4">
              {t("elections.completed")} (2024-2025)
            </h3>
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 text-left">
              {[
                ...
                <div
                  key={e.state}
                  className="p-4 rounded-xl border border-[var(--border-color)] bg-[var(--bg-glass)] hover:border-saffron-500/30 transition-all opacity-70"
                >
                  ...
                  <p className="text-sm text-[var(--text-secondary)]">
                    {e.seats} {t("elections.seats")} - {e.year}
                  </p>
                </div>
              ))}
            </div>
      ...
      {(activeTab === "By-Elections" || activeTab === "Local Body") && (
        <div className="glass-card p-8 text-center animate-fade-up">
          <h2 className="text-xl font-bold text-[var(--text-primary)] mb-2">
            {t(
              `elections.${activeTab === "By-Elections" ? "byElections" : "localBody"}`,
            )}
          </h2>
          <p className="text-[var(--text-secondary)] max-w-md mx-auto">
            {activeTab === "By-Elections"
              ? language === "en"
                ? "By-election data for vacancies due to death, resignation, or disqualification of sitting members."
                : "सत्तारूढ़ सदस्यों की मृत्यु, इस्तीफे या अयोग्यता के कारण रिक्तियों के लिए उपचुनाव डेटा।"
              : language === "en"
                ? "Municipal, Panchayat, and local body election data organized by state."
                : "राज्य द्वारा आयोजित नगरपालिका, पंचायत और स्थानीय निकाय चुनाव डेटा।"}
          </p>
          <p className="text-sm text-[var(--text-muted)] mt-4">
            {t("elections.dataSoon")}
          </p>
        </div>
      )}
    </div>
