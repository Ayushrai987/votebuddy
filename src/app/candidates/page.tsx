...
export default function CandidatesPage() {
  ...
  const uniqueStates = Array.from(
    new Set(sampleCandidates.map((c) => c.state)),
  );
  const uniqueParties = Array.from(
    new Set(sampleCandidates.map((c) => c.partyShort)),
  );
      ...
      <div className="glass-card p-4 sm:p-6 mb-8">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-3">
          ...
          <select
            value={resultFilter}
            onChange={(e) =>
              setResultFilter(e.target.value as "" | "Won" | "Lost")
            }
            className="px-3 py-2.5 rounded-xl border border-[var(--border-color)] bg-[var(--bg-input)] text-[var(--text-primary)] text-sm focus:outline-none focus:border-saffron-500 transition-all"
          >
        ...
        <div className="flex items-center justify-between mt-3 text-xs text-[var(--text-muted)]">
          <span>
            {t("candidates.showing")} {candidates.length} {t("candidates.of")}{" "}
            {sampleCandidates.length} {t("candidates.candidates")}
          </span>
          {compareIds.length > 0 && (
            <button
      ...
      {compareList.length >= 2 && (
        <div className="glass-card p-6 mb-8 animate-fade-up">
          <h2 className="text-lg font-extrabold text-[var(--text-primary)] mb-4">
            {t("candidates.compareTitle")}
          </h2>
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-[var(--border-color)]">
                  <th className="text-left py-2 px-3 text-xs text-[var(--text-muted)] uppercase">
                    {t("candidates.metric")}
                  </th>
                  {compareList.map((c) => (
                    <th
                      key={c.id}
                      className="text-left py-2 px-3 text-xs font-bold text-[var(--text-primary)]"
                    >
                      {c.name}
                    </th>
                  ))}
              ...
              <tbody>
                {[
                  {
                    label: t("candidates.party"),
                    get: (c: any) => c.partyShort,
                  },
                  {
                    label: t("candidates.constituency"),
                    get: (c: any) => c.constituency,
                  },
                  { label: t("candidates.state"), get: (c: any) => c.state },
                  {
                    label: t("candidates.age"),
                    get: (c: any) => c.age.toString(),
                  },
                  {
                    label: t("candidates.education"),
                    get: (c: any) => c.education || "N/A",
                  },
                  {
                    label: t("candidates.criminalCases"),
                    get: (c: any) => c.criminalCases?.toString() || "0",
                  },
                  {
                    label: t("candidates.assets"),
                    get: (c: any) => c.assets || "N/A",
                  },
                  {
                    label: t("candidates.votes"),
                    get: (c: any) => c.votes?.toLocaleString() || "N/A",
                  },
                  {
                    label: t("candidates.margin"),
                    get: (c: any) =>
                      c.margin
                  ...
                  <tr
                    key={row.label}
                    className="border-b border-[var(--border-color)] hover:bg-[var(--bg-glass)]"
                  >
                    <td className="py-2 px-3 text-xs font-semibold text-[var(--text-muted)]">
                      {row.label}
                    </td>
                    {compareList.map((c) => (
                      <td
                        key={c.id}
                        className="py-2 px-3 text-sm text-[var(--text-secondary)]"
                      >
                        {row.get(c)}
                      </td>
                    ))}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        {candidates.map((c) => {
          ...
          return (
            <div
              key={c.id}
              className={`glass-card overflow-hidden transition-all ${isComparing ? "ring-2 ring-saffron-500" : ""}`}
            >
              {/* Party color bar */}
              <div
                className="h-1.5"
              ...
              <div className="p-5">
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <Link
                        ...
                        className="text-base font-bold text-[var(--text-primary)] hover:text-saffron-500 transition-colors"
                      >
                        {c.name}
                      </Link>
                      <p className="text-xs text-[var(--text-muted)]">
                        {c.partyShort} - {c.constituency}
                      </p>
                    </div>
                  </div>
                  <span
                    className={`pill-badge text-[0.6rem] ${c.result === "Won" ? "pill-badge-green" : "pill-badge-accent"}`}
                  >
                    {c.result === "Won"
                      ? t("candidates.won")
                      : t("candidates.lost")}
                ...
                <div className="grid grid-cols-2 gap-2 text-xs mb-3">
                  ...
                  <div className="p-2 rounded-lg bg-[var(--bg-glass)] border border-[var(--border-color)]">
                    ...
                    <p
                      className={`font-bold ${(c.margin || 0) > 0 ? "text-green-500" : "text-red-500"}`}
                    >
                      {c.margin
                        ? (c.margin > 0 ? "+" : "") + c.margin.toLocaleString()
                        : "N/A"}
                  ...
                  <div className="p-2 rounded-lg bg-[var(--bg-glass)] border border-[var(--border-color)]">
                    ...
                    <p
                      className={`font-bold ${(c.criminalCases || 0) > 0 ? "text-red-500" : "text-green-500"}`}
                    >
                      {c.criminalCases || 0}
                    </p>
                  </div>
                ...
                <div className="flex items-center justify-between pt-3 border-t border-[var(--border-color)]">
                  ...
                  <div className="flex gap-2">
                    ...
                    <Link
                      href={`/candidates/${c.id}`}
                      className="text-xs text-saffron-500 font-semibold hover:underline px-2 py-1"
                    >
                      {t("candidates.view")}
                    </Link>
                  </div>
