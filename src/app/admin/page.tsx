...
export default function AdminPage() {
  ...
  useEffect(() => {
    if (user?.email) {
      ...
      const isWhitelisted =
        ADMIN_WHITELIST.includes(user.email) || envEmails.includes(user.email);
      ...
      <div className="page-enter mx-auto max-w-7xl px-4 sm:px-6 py-12 flex flex-col items-center justify-center min-h-[60vh]">
        <div className="glass-card p-8 text-center max-w-md w-full">
          ...
          <p className="text-[var(--text-secondary)] mb-8">
            Please sign in with an authorized Google account to access platform
            management tools.
          </p>
          <button
    ...
    return (
      <div className="page-enter mx-auto max-w-7xl px-4 sm:px-6 py-12 flex flex-col items-center justify-center min-h-[60vh]">
        <div className="glass-card p-8 text-center max-w-md w-full border-red-500/30">
          <h1 className="text-2xl font-black text-red-500 mb-4">
            Access Denied
          </h1>
          ...
          <button onClick={logout} className="btn-ghost w-full py-3 rounded-xl">
    ...
    <div className="page-enter mx-auto max-w-7xl px-4 sm:px-6 py-8 sm:py-12">
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-10">
        <div>
          ...
          <p className="text-[var(--text-secondary)]">
            Welcome back, {user.displayName?.split(" ")[0] || "Admin"}. Manage
            data and monitor systems.
          </p>
        </div>
        ...
        <div className="flex items-center gap-3">
          ...
          <img
            src={user.photoURL || ""}
            alt="Admin"
            className="w-10 h-10 rounded-full border-2 border-saffron-500 p-0.5"
          />
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-10">
        {[
          ...
          <div
            key={stat.label}
            className="glass-card p-5 group hover:border-saffron-500/50 transition-all"
          >
        ...
        {[
          {
            icon: "",
            title: "News Curation",
            desc: "Add, edit, or flag election news and fact-checks.",
            action: "Launch Tool",
          },
          {
            icon: "",
            title: "Announcements",
            desc: "Post global alerts to the home page ticker.",
            action: "Manage",
          },
          {
            icon: "",
            title: "Data Review",
            desc: "Validate constituency and candidate updates.",
            action: "Review",
          },
          {
            icon: "",
            title: "Analytics",
            desc: "View deep engagement metrics and heatmaps.",
            action: "Open Dashboard",
          },
          {
            icon: "",
            title: "Access Control",
            desc: "Manage admin whitelist and permissions.",
            action: "Configure",
          },
          {
            icon: "",
            title: "System Config",
            desc: "Update API keys and platform constants.",
            action: "Settings",
          },
        ].map((tool) => (
