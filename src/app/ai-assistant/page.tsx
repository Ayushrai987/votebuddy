...
const INITIAL_MESSAGE_EN: ChatMessage = {
  ...
  role: "assistant",
  content:
    "Namaste! I am VoteBuddy, your AI Election Assistant. How can I help you today? You can ask me about:\n\n*   Your polling booth location\n*   Voter registration process\n*   Election dates for your state\n*   ECI guidelines and rules\n*   Candidate information",
  timestamp: Date.now(),
};
...
export default function AIAssistantPage() {
  ...
  useEffect(() => {
    if (messages.length === 0) {
      setMessages([
        language === "en" ? INITIAL_MESSAGE_EN : INITIAL_MESSAGE_HI,
      ]);
    } else {
      // Switch initial message if language changes and it's the only message
      if (
        messages.length === 1 &&
        (messages[0].id === "init" || messages[0].id === "init-hi")
      ) {
        setMessages([
          language === "en" ? INITIAL_MESSAGE_EN : INITIAL_MESSAGE_HI,
        ]);
  ...
  const suggestions =
    language === "en" ? SUGGESTED_QUESTIONS_EN : SUGGESTED_QUESTIONS_HI;
      ...
      <div className="space-y-4">
        ...
        <form onSubmit={handleSubmit} className="relative">
          <input
            ...
            placeholder={
              language === "en"
                ? "Ask about elections, voter ID, or ECI rules..."
                : "चुनाव, वोटर आईडी, या ईसीआई नियमों के बारे में पूछें..."
            }
