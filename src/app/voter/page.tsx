...
export default function VoterServicesPage() {
  ...
  const [eligibilityResult, setEligibilityResult] = useState<string | null>(
    null,
  );
  ...
  function checkEligibility() {
    ...
    const age = Math.floor(
      (qualifyingDate.getTime() - dob.getTime()) /
        (365.25 * 24 * 60 * 60 * 1000),
    );

    if (age >= 18) {
      setEligibilityResult(
        t("voter.eligibleMsg").replace("{age}", age.toString()),
      );
    } else {
      const turnDate = new Date(dob);
      turnDate.setFullYear(turnDate.getFullYear() + 18);
      const dateStr = turnDate.toLocaleDateString(
        language === "en" ? "en-IN" : "hi-IN",
        { day: "numeric", month: "long", year: "numeric" },
      );
      setEligibilityResult(
        t("voter.notEligibleMsg").replace("{date}", dateStr),
      );
    }
  }
  ...
  return (
    <div className="page-enter mx-auto max-w-7xl px-4 sm:px-6 py-8 sm:py-12">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3 mb-10">
        ...
        <a
          href="https://voters.eci.gov.in/download-epic"
          target="_blank"
          rel="noopener noreferrer"
          className="glass-card-hover p-4 text-center"
        >
            ...
            <div className="glass-card p-6 sm:p-8 animate-fade-up">
              <div className="flex items-center gap-3 mb-4">
                ...
                <div>
                  <h2 className="text-xl font-extrabold text-[var(--text-primary)]">
                    {language === "en" ? reg.form6.title : reg.form6.titleHi}
                  </h2>
                </div>
              </div>
              <div className="flex items-center justify-between gap-4 mb-6">
                <p className="text-sm text-[var(--text-secondary)]">
                  {language === "en"
                    ? reg.form6.summary
                    : "नए मतदाताओं के पंजीकरण के लिए फॉर्म।"}
                </p>
                <button
              ...
              <div className="info-card">
                <h4 className="text-xs font-bold uppercase tracking-wider text-saffron-500 mb-2">
                  {t("voter.qualifyingDates")}
                </h4>
                <div className="flex flex-wrap gap-2">
                  {reg.form6.qualifyingDates.map((d) => (
                    <span key={d} className="pill-badge pill-badge-accent">
                      {d}
                    </span>
                  ))}
                </div>
                <p className="text-xs text-[var(--text-muted)] mt-2">
                  {t("voter.qualifyingDesc")}
                </p>
              </div>
            </div>
          )}
          ...
          {activeSection === "form7" && (
            <div className="glass-card p-6 sm:p-8 animate-fade-up">
              <div className="flex items-center gap-3 mb-4">
                ...
                <div>
                  <h2 className="text-xl font-extrabold text-[var(--text-primary)]">
                    {language === "en" ? reg.form7.title : reg.form7.titleHi}
                  </h2>
                </div>
              </div>
              <ul className="space-y-3">
                {reg.form7.details.map((d, i) => (
                  <li
                    ...
                    className="flex items-start gap-2 text-sm text-[var(--text-secondary)]"
                  >
                    <span className="text-saffron-500 flex-shrink-0" />
                    {d}
                  </li>
                ))}
              </ul>
          ...
          {activeSection === "form8" && (
            <div className="glass-card p-6 sm:p-8 animate-fade-up">
              <div className="flex items-center gap-3 mb-4">
                ...
                <div>
                  <h2 className="text-xl font-extrabold text-[var(--text-primary)]">
                    {language === "en" ? reg.form8.title : reg.form8.titleHi}
                  </h2>
                </div>
              </div>
              <ul className="space-y-3">
          ...
          {activeSection === "epic" && (
            <div className="glass-card p-6 sm:p-8 animate-fade-up">
              <div className="flex items-center gap-3 mb-4">
                ...
                <div>
                  <h2 className="text-xl font-extrabold text-[var(--text-primary)]">
                    {language === "en" ? reg.epic.title : reg.epic.titleHi}
                  </h2>
                </div>
              </div>
              ...
              <h3 className="section-heading mb-3">
                {t("voter.alternativeIds")}
              </h3>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                {reg.epic.alternativeIds.map((id, i) => (
                  <div
                    key={i}
                    className="flex items-center gap-2 text-sm text-[var(--text-secondary)] p-2 rounded-lg bg-[var(--bg-glass)] border border-[var(--border-color)]"
                  >
          ...
          {activeSection === "helpline" && (
            <div className="glass-card p-6 sm:p-8 animate-fade-up">
              ...
              <div className="text-center py-6 mb-6">
                <a
                  href="tel:1950"
                  className="inline-flex items-center gap-3 px-8 py-4 rounded-2xl bg-gradient-to-r from-saffron-500 to-saffron-400 text-white text-xl font-bold shadow-lg shadow-saffron-500/30 hover:shadow-saffron-500/50 transition-all hover:scale-105"
                >
                  {t("voter.callNow")}
                </a>
          ...
          {activeSection === "eligibility" && (
            <div className="glass-card p-6 sm:p-8 animate-fade-up">
              ...
              <div className="max-w-sm mx-auto text-center py-6">
                ...
                {eligibilityResult && (
                  <div
                    className={`mt-6 p-4 rounded-xl border text-sm text-left leading-relaxed ${
                      eligibilityResult.includes(
                        t("voter.eligibleMsg").substring(0, 10),
                      )
