import { initGoogleServices, logAnalyticsEvent, getAnalyticsInstance, getPerformanceInstance } from './firebase';
...
describe('Firebase Integration', () => {
  ...
  describe('initGoogleServices', () => {
    ...
    it('handles initialization errors gracefully', async () => {
      ...
      await initGoogleServices();
      expect(consoleSpy).toHaveBeenCalledWith('Error initializing Google services:', expect.any(Error));
  ...
  describe('logAnalyticsEvent', () => {
    it('logs events correctly when initialized', async () => {
      ...
      expect(logEvent).toHaveBeenCalledWith(expect.anything(), 'test_event', { param: 'value' });
    });
  });
