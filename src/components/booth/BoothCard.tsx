  ...
  return (
    ...
    >
      ...
      <p className="text-xs text-[var(--text-secondary)] mb-2 flex items-start gap-1">
        <MapPin className="w-3.5 h-3.5 mt-0.5 flex-shrink-0 text-red-500" aria-hidden="true" />
        <span>{booth.address}</span>
      </p>
      
      {/* Accessibility badges */}
      <div className="flex flex-wrap gap-1 mt-2" data-testid="accessibility-badges" role="list" aria-label="Booth accessibility features">
        {booth.accessibility.ramp && (
          <span className="text-[10px] px-1.5 py-0.5 rounded bg-green-500/10 text-green-600 border border-green-500/20" role="listitem">
            {t('booth.accessibility.ramp')}
          </span>
        )}
        {booth.accessibility.wheelchair && (
          <span className="text-[10px] px-1.5 py-0.5 rounded bg-indigo-500/10 text-indigo-600 border border-indigo-500/20" role="listitem">
            {t('booth.accessibility.wheelchair') || 'Wheelchair'}
          </span>
        )}
        {booth.accessibility.drinkingWater && (
          <span className="text-[10px] px-1.5 py-0.5 rounded bg-blue-500/10 text-blue-600 border border-blue-500/20" role="listitem">
            {t('booth.accessibility.water')}
          </span>
        )}
        {booth.accessibility.toilets && (
          <span className="text-[10px] px-1.5 py-0.5 rounded bg-purple-500/10 text-purple-600 border border-purple-500/20" role="listitem">
            {t('booth.accessibility.toilet')}
          </span>
        )}
        {booth.accessibility.shade && (
          <span className="text-[10px] px-1.5 py-0.5 rounded bg-amber-500/10 text-amber-600 border border-amber-500/20" role="listitem">
